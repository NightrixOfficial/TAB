...
- Players are sorted in the tab list by LuckPerms group weight - whichever group
  someone's in with the highest weight counts as their highest role, and higher
  roles sort to the top. This is always on (no command needed) as long as
  LuckPerms is installed; see `luckperms.sort-by-weight` in config.yml to turn
  it off.
...
luckperms:
  ...
  sort-by-weight: true   # sort tab list by highest LuckPerms group weight
...
header-image:
  enabled: false
  show-divider: true      # extra purple gradient line just below the banner
  port: 25566
  public-address: ""
  required: false
  prompt: "..."
...
The purple lines in the footer (and the one under the banner, when
`header-image.show-divider` is on) aren't text characters like `---` - the
tab list panel only ever sizes itself to fit its widest line, so a row of
dashes stays exactly as wide as the dashes and never "reaches" the edges.
Instead the dividers are drawn the same way the logo banner is: a single
custom-font character mapped to a wide gradient-bar image (see
`tools/generate_pack.py`, `build_divider()`). Because that line is now the
widest thing in the whole panel, the client sizes the tab list to match it -
which is what makes it look like it spans "the entire tab list." If you ever
want it wider or narrower, adjust `DIVIDER_WIDTH_PX` in that script and
re-run it to rebuild `pack.zip`.
