...
public final class PixelWidth {
    ...
    public static int widthOf(String plainText, boolean bold) {
        ...
        for (int i = 0; i < plainText.length(); i++) {
            total += WIDTHS.getOrDefault(plainText.charAt(i), DEFAULT_WIDTH) + (bold ? 1 : 0);
        }
        return total;
    }

    /**
     * Measures a MiniMessage-style template string (tags like {@code
     * <bold>}/{@code </bold>}, {@code <white>}, etc. still in it, with
     * placeholders already substituted with their plain text). Every
     * {@code <...>} tag is skipped over rather than measured - it's not
     * visible text - except {@code <bold>}/{@code </bold>} (and the {@code
     * <b>}/{@code </b>} shorthand), which are tracked so characters between
     * them get bold's extra 1px, matching whatever bold state the format
     * string itself puts around a given stretch of text.
     */
    public static int widthOfTemplate(String template) {
        if (template == null || template.isEmpty()) {
            return 0;
        }
        String text = stripCodes(template);
        int total = 0;
        boolean bold = false;
        int i = 0;
        int len = text.length();
        while (i < len) {
            ...
            if (c == '<') {
                ...
                if (end == -1) {
                    total += widthOf(text.substring(i), bold);
                    break;
                }
                String tag = text.substring(i + 1, end).toLowerCase();
            ...
            total += WIDTHS.getOrDefault(c, DEFAULT_WIDTH) + (bold ? 1 : 0);
            i++;
        }
        return total;
    }
